      ...
      <transition name="toup">
        <div class="vue-picker" ref="picker" v-show="visible">
          <Header v-if="showToolbar"
            ...
            @cancel="cancel"
            @confirm="confirm" />
          <div class="content" :style="{height: boxHeight + 'rem'}">
            <div class="colums">
              <List :column="column1"
                :boxHeight="boxHeight"
            ...
            <div class="hairline" :style="{'--h':`${itemHeight}rem`}"></div>
          </div>
        </div>
      </transition>
    </div>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      maskStyle() {
        let style = { backgroundSize: `100% ${(this.boxHeight-this.itemHeight)/2}rem` }
        // if (this.getRowNumber === 3) {
        //   style = { backgroundSize: '100% .88rem' }
        // }
        return style
      }
    },
        ...
        this.indexArr = [
          () => {
            ...
            if(this.data&&this.data.length>0){
              this.result[0]=this.data[0][this.dIndex1]
            }
          },
          () => {
            this.dIndex2 = this.defaultIndex[1] || 0;
            if(this.data&&this.data.length>1){
              this.result[1]=this.data[1][this.dIndex2]
            }
          },
          () => {
            this.dIndex3 = this.defaultIndex[2] || 0;
            if(this.data&&this.data.length>2){
              this.result[2]=this.data[2][this.dIndex3]
            }
          },
          () => {
            this.dIndex4 = this.defaultIndex[3] || 0;
            if(this.data&&this.data.length>3){
              this.result[3]=this.data[3][this.dIndex4]
            }
          }
        ]
        this.formateData()

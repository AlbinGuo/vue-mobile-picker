...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      base(){
        return Number(document.documentElement.style["font-size"].replace("px",""));
      }
    },
    methods: {
      init () {
        ...
        this.bottom = this.bottom*this.base;
        this.top = this.top*this.base;
        console.log(this.bottom,this.top,this.base)
      },
      // 根据index 设置滚动位置
      setTop (index = 0) {
        const { boxHeight, itemHeight } = this
        this.startTop = ((boxHeight - itemHeight) / 2) - (index * itemHeight)
        this.startTop = this.startTop*this.base;
      ...
      handleEnd () {
        ...
        console.log("end","startTop="+this.startTop,"momentumTop"+this.momentumTop,"distance="+distance,"duration="+duration,"allowMomentum="+allowMomentum)
      ...
      setTranfromTop () {
        ...
        console.log("setTranfromTop","startTop="+this.startTop,"bottom="+this.bottom,"itemHeight")
        let itemHeight = this.itemHeight*this.base;
        if (this.startTop >= this.bottom - itemHeight) {
          this.setTop()
        } else if (this.startTop <= this.top + itemHeight) {
          this.setTop(this.count - 1)
        } else {
          let index = Math.round((this.startTop) / itemHeight)
          this.startTop = index * itemHeight
          if (this.startTop > this.bottom) {
            this.startTop = this.bottom - itemHeight
            index = -this.getRoNumber
          } else if (this.startTop < this.top) {
            this.startTop = this.top + itemHeight
            index = this.count + 1
          }
          this.ulStyle.transform = `translate3d(0px, ${this.startTop}px, 0px)`
